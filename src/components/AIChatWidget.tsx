...
export function AIChatWidget() {
  ...
  return (
    <>
      ...
      {open && (
        <div dir={dir} className="fixed bottom-20 right-4 z-50 flex h-[70vh] max-h-[600px] w-[calc(100vw-32px)] max-w-sm flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-2xl">
          <div className="flex items-center gap-2 border-b border-border bg-gradient-to-r from-primary/10 to-primary/5 p-3">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground">{t("ai_subtitle")}</div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-auto p-3 space-y-3">
            ...
            {messages.map(m => (
              <div key={m.id} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[80%] rounded-2xl px-3 py-2 text-sm whitespace-pre-wrap ${m.role === "user" ? "bg-primary text-primary-foreground" : "bg-muted"}`}>
          ...
          <form onSubmit={e => { e.preventDefault(); send(); }} className="flex gap-2 border-t border-border p-3">
            <Input value={input} onChange={e => setInput(e.target.value)} placeholder={t("ai_placeholder")} autoFocus />
